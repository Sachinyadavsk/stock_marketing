...
function timeAgo($dateStr) {
    $date = DateTime::createFromFormat("d/m/Y h:i:s a", $dateStr);
    $now = new DateTime();
    $diff = $now->diff($date);

    if ($diff->y) return $diff->y . ' year' . ($diff->y > 1 ? 's' : '') . ($date < $now ? ' ago' : ' from now');
    if ($diff->m) return $diff->m . ' month' . ($diff->m > 1 ? 's' : '') . ($date < $now ? ' ago' : ' from now');
    if ($diff->d >= 7) {
        $w = floor($diff->d / 7);
        return $w . ' week' . ($w > 1 ? 's' : '') . ($date < $now ? ' ago' : ' from now');
    }
    if ($diff->d) return $diff->d . ' day' . ($diff->d > 1 ? 's' : '') . ($date < $now ? ' ago' : ' from now');
    if ($diff->h) return $diff->h . ' hour' . ($diff->h > 1 ? 's' : '') . ($date < $now ? ' ago' : ' from now');
    if ($diff->i) return $diff->i . ' minute' . ($diff->i > 1 ? 's' : '') . ($date < $now ? ' ago' : ' from now');
    return 'just now';
}
