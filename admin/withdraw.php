...
<?php 
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ids'], $_POST['action'])) {
        $ids = array_map('intval', $_POST['ids']);
        $action = $_POST['action'];

        if (in_array($action, ['success', 'cancel']) && !empty($ids)) {
            $status = $action === 'success' ? 'success' : 'cancel';
            $ids_str = implode(',', $ids);
            $update_sql = "UPDATE withdrawal SET status = '$status' WHERE id IN ($ids_str)";
        ...
        <div class="content">
            <div class="container-xl">
                <div class="">
                    <div class="page-header">
                        <div class="row align-items-center">
                            <div class="col-auto">
                                <h2 class="page-title">All Withdraw History</h2>
                            </div>
                        </div>
                ...
                <?php
                        ...
                        $conditions = ["withdrawal.status = 'pending'"];
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="card">
                                    <div class="card-header"><h3 class="card-title">Withdraw History</h3></div>
                                    ...
                                    <form method="post" action="">
                                                    ...
                                                    <?php if (mysqli_num_rows($result) > 0): ?>
                                                        <?php while ($row = mysqli_fetch_assoc($result)): ?>
                                                            <tr>
                                                                ...
                                                                <td class="text-warning"><?= htmlspecialchars($row['status']) ?></td>
                                        ...
                                        <div class="card-footer">
                                            <button type="submit" name="action" value="success" class="btn btn-success btn-sm" onclick="return confirm('Success selected withdrawals?')">Success Selected</button>
                                            <button type="submit" name="action" value="cancel" class="btn btn-danger btn-sm" onclick="return confirm('Cancel selected withdrawals?')">Cancel Selected</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
